  ...
  group "atlas-web" {
    ...
    service {
      ...
      tags = [
        ...
        "traefik.http.routers.nginx-atlas.middlewares=redirect-user-web,atlas-www-redirect",
        # redirect redbrick.dcu.ie/~user to user.redbrick.dcu.ie
        "traefik.http.middlewares.redirect-user-web.redirectregex.regex=^https?://(?:www\\.)?redbrick\\.dcu\\.ie/~([^/]+)(/.*)?$",
